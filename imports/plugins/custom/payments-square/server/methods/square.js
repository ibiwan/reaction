...
Meteor.methods({
   ...
   */
  "squareSubmit": function (transactionType, cardData, paymentData) {
    ...
    if (cardData.cardNonce === "") {
      throw new Error("Invalid CardNonce Value");
    }
   ...
   */
  "square/refund/create": function (paymentMethod, amount) {
    check(paymentMethod, Reaction.Schemas.PaymentMethod);
    check(amount, Number);
    const { transactionId } = paymentMethod;
    const response = SquareApi.methods.refund.call({
   ...
   * @return {Object} result
   */
  "square/refund/list": function (paymentMethod) {
    check(paymentMethod, Reaction.Schemas.PaymentMethod);
    const { transactionId } = paymentMethod;
    const response = SquareApi.methods.refunds.call({
      transactionId: transactionId
    });
    const result = [];
